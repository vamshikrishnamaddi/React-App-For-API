Liferay.Loader.define("my-react-app$email-addresses@5.0.0/lib/email-addresses.min", ['module', 'exports', 'require'], function (module, exports, require) {
    ...
    (function (global) {
      ...
      function parse5322(opts) {
        function inStr() {
          ...
          pos = 0;len = parseString.length;
        }function o(name, value) {
          return { name: name, tokens: value || "", semantic: value || "", children: [] };
        }function wrap(name, ast) {
          var n;if (ast === null) {
            return null;
          }n = o(name);n.tokens = ast.tokens;n.semantic = ast.semantic;n.children.push(ast);return n;
        }function add(parent, child) {
          if (child !== null) {
            parent.tokens += child.tokens;parent.semantic += child.semantic;
          }parent.children.push(child);return parent;
        }function compareToken(fxnCompare) {
          var tok;if (!inStr()) {
            return null;
          }tok = curTok();if (fxnCompare(tok)) {
            nextTok();return o("token", tok);
          }return null;
        }function noop() {
          return null;
          ...
          var args = arguments;return function andFunc() {
            var i, s, result, start;start = getPos();s = o("and");for (i = 0; i < args.length; i += 1) {
              result = args[i]();if (result === null) {
                setPos(start);return null;
              }add(s, result);
            }return s;
          };
        }function or() {
          var args = arguments;return function orFunc() {
            var i, result, start;start = getPos();for (i = 0; i < args.length; i += 1) {
              result = args[i]();if (result !== null) {
                return result;
              }setPos(start);
            }return null;
          ...
          return function optFunc() {
            var result, start;start = getPos();result = prod();if (result !== null) {
              return result;
            } else {
              setPos(start);return o("opt");
            }
          };
        }function invis(prod) {
          return function invisFunc() {
            var result = prod();if (result !== null) {
              result.semantic = "";
            }return result;
          };
        }function colwsp(prod) {
          return function collapseSemanticWhitespace() {
            var result = prod();if (result !== null && result.semantic.length > 0) {
              result.semantic = " ";
            }return result;
          };
        }function star(prod, minimum) {
          return function starFunc() {
            var s, result, count, start, min;start = getPos();s = o("star");count = 0;min = minimum === undefined ? 0 : minimum;while ((result = prod()) !== null) {
              count = count + 1;add(s, result);
            }if (count >= min) {
              return s;
            } else {
              setPos(start);return null;
          ...
          return wrap("vchar", compareToken(function vcharFunc(tok) {
            var code = tok.charCodeAt(0);var accept = 33 <= code && code <= 126;if (opts.rfc6532) {
          ...
          var qp = wrap("quoted-pair", or(and(literal("\\"), or(vchar, wsp)), obsQP)());if (qp === null) {
            return null;
          }qp.semantic = qp.semantic[1];return qp;
        }function fws() {
          ...
          return wrap("ctext", or(function ctextFunc1() {
            return compareToken(function ctextFunc2(tok) {
              var code = tok.charCodeAt(0);var accept = 33 <= code && code <= 39 || 42 <= code && code <= 91 || 93 <= code && code <= 126;if (opts.rfc6532) {
                accept = accept || isUTF8NonAscii(tok);
              }return accept;
            });
          }, obsCtext)());
          ...
          return wrap("atext", compareToken(function atextFunc(tok) {
            var accept = "a" <= tok && tok <= "z" || "A" <= tok && tok <= "Z" || "0" <= tok && tok <= "9" || ["!", "#", "$", "%", "&", "'", "*", "+", "-", "/", "=", "?", "^", "_", "`", "{", "|", "}", "~"].indexOf(tok) >= 0;if (opts.rfc6532) {
              accept = accept || isUTF8NonAscii(tok);
            }return accept;
          }));
        }function atom() {
          ...
          var s, maybeText;s = wrap("dot-atom-text", star(atext, 1)());if (s === null) {
            return s;
          }maybeText = star(and(literal("."), star(atext, 1)))();if (maybeText !== null) {
            add(s, maybeText);
          }return s;
        }function dotAtom() {
          ...
          return wrap("qtext", or(function qtextFunc1() {
            return compareToken(function qtextFunc2(tok) {
              var code = tok.charCodeAt(0);var accept = 33 === code || 35 <= code && code <= 91 || 93 <= code && code <= 126;if (opts.rfc6532) {
                accept = accept || isUTF8NonAscii(tok);
              }return accept;
            });
          }, obsQtext)());
          ...
          return wrap("display-name", function phraseFixedSemantic() {
            var result = phrase();if (result !== null) {
          ...
          return wrap("address-list", or(and(address, star(and(literal(opts.addressListSeparator), address))), obsAddrList)());
        }function groupList() {
          ...
          return wrap("dtext", or(function dtextFunc1() {
            return compareToken(function dtextFunc2(tok) {
              var code = tok.charCodeAt(0);var accept = 33 <= code && code <= 90 || 94 <= code && code <= 126;if (opts.rfc6532) {
                accept = accept || isUTF8NonAscii(tok);
              }return accept;
            });
          }, obsDtext)());
          ...
          return wrap("domain", function domainCheckTLD() {
            var result = or(obsDomain, dotAtom, domainLiteral)();if (opts.rejectTLD) {
          ...
          return opts.strict ? null : wrap("obs-NO-WS-CTL", compareToken(function (tok) {
            var code = tok.charCodeAt(0);return 1 <= code && code <= 8 || 11 === code || 12 === code || 14 <= code && code <= 31 || 127 === code;
          }));
        }function obsCtext() {
          ...
          return opts.strict ? null : wrap("obs-phrase", and(word, star(or(word, literal("."), opts.atInDisplayName ? literal("@") : noop, opts.commaInDisplayName ? literal(",") : noop, colwsp(cfws))))());
        }function obsFws() {
          ...
          var i, stack, node;if (root === null || root === undefined) {
            return null;
          }stack = [root];while (stack.length > 0) {
            node = stack.pop();if (node.name === name) {
              return node;
          ...
          var i, stack, node, result;if (root === null || root === undefined) {
            return null;
          }stack = [root];result = [];while (stack.length > 0) {
            node = stack.pop();if (node.name === name) {
              result.push(node);
            }for (i = node.children.length - 1; i >= 0; i -= 1) {
              stack.push(node.children[i]);
          ...
          var i, stack, node, result, namesLookup;if (root === null || root === undefined) {
            return null;
          }stack = [root];result = [];namesLookup = {};for (i = 0; i < names.length; i += 1) {
            namesLookup[names[i]] = true;
          }while (stack.length > 0) {
            node = stack.pop();if (node.name in namesLookup) {
              result.push(node);
            } else {
              for (i = node.children.length - 1; i >= 0; i -= 1) {
                stack.push(node.children[i]);
          ...
          var addresses, groupsAndMailboxes, i, groupOrMailbox, result;if (ast === null) {
            return null;
          }addresses = [];groupsAndMailboxes = findAllNodesNoChildren(["group", "mailbox"], ast);for (i = 0; i < groupsAndMailboxes.length; i += 1) {
            groupOrMailbox = groupsAndMailboxes[i];if (groupOrMailbox.name === "group") {
          ...
          }result = { ast: ast, addresses: addresses };if (opts.simple) {
          ...
          var i;var groupName = findNode("display-name", group);var groupResultMailboxes = [];var mailboxes = findAllNodesNoChildren(["mailbox"], group);for (i = 0; i < mailboxes.length; i += 1) {
            groupResultMailboxes.push(giveResultMailbox(mailboxes[i]));
          }return { node: group, parts: { name: groupName }, type: group.name, name: grabSemantic(groupName), addresses: groupResultMailboxes };
        }function giveResultMailbox(mailbox) {
          var name = findNode("display-name", mailbox);var aspec = findNode("addr-spec", mailbox);var cfws = findAllNodes("cfws", mailbox);var comments = findAllNodesNoChildren(["comment"], mailbox);var local = findNode("local-part", aspec);var domain = findNode("domain", aspec);return { node: mailbox, parts: { name: name, address: aspec, local: local, domain: domain, comments: cfws }, type: mailbox.name, name: grabSemantic(name), address: grabSemantic(aspec), local: grabSemantic(local), domain: grabSemantic(domain), comments: concatComments(comments), groupName: grabSemantic(mailbox.groupName) };
        }function grabSemantic(n) {
          return n !== null && n !== undefined ? n.semantic : null;
        }function simplifyResult(result) {
          var i;if (result && result.addresses) {
            for (i = 0; i < result.addresses.length; i += 1) {
              delete result.addresses[i].node;
            }
          }return result;
        }function concatComments(comments) {
          var result = "";if (comments) {
        ...
        }var parseString, pos, len, parsed, startProduction;opts = handleOpts(opts, {});if (opts === null) {
          return null;
        }parseString = opts.input;startProduction = { address: address, "address-list": addressList, "angle-addr": angleAddr, from: function fromSpec() {
            return wrap("from", or(mailboxList, addressList)());
          }, group: group, mailbox: mailbox, "mailbox-list": mailboxList, "reply-to": function replyToSpec() {
            return wrap("reply-to", addressList());
          }, sender: function senderSpec() {
            return wrap("sender", or(mailbox, address)());
          } }[opts.startAt] || addressList;if (!opts.strict) {
          initialize();opts.strict = true;parsed = startProduction(parseString);if (opts.partial || !inStr()) {
            return giveResult(parsed);
          }opts.strict = false;
        }initialize();parsed = startProduction(parseString);if (!opts.partial && inStr()) {
          return null;
        }return giveResult(parsed);
        ...
        function isString(str) {
        ...
        }var defaults, o;if (isString(opts)) {
          opts = { input: opts };
          ...
          return null;
        }defaults = { oneResult: false, partial: false, rejectTLD: false, rfc6532: false, simple: false, startAt: "address-list", strict: false, atInDisplayName: false, commaInDisplayName: false, addressListSeparator: "," };for (o in defaults) {
          if (isNullUndef(opts[o])) {
            opts[o] = !isNullUndef(defs[o]) ? defs[o] : defaults[o];
          }
        }return opts;
      }parse5322.parseOneAddress = function parseOneAddressSimple(opts) {
        return parse5322(handleOpts(opts, { oneResult: true, rfc6532: true, simple: true, startAt: "address-list" }));
      };parse5322.parseAddressList = function parseAddressListSimple(opts) {
        return parse5322(handleOpts(opts, { rfc6532: true, simple: true, startAt: "address-list" }));
      };parse5322.parseFrom = function parseFromSimple(opts) {
        return parse5322(handleOpts(opts, { rfc6532: true, simple: true, startAt: "from" }));
      };parse5322.parseSender = function parseSenderSimple(opts) {
        return parse5322(handleOpts(opts, { oneResult: true, rfc6532: true, simple: true, startAt: "sender" }));
      };parse5322.parseReplyTo = function parseReplyToSimple(opts) {
        return parse5322(handleOpts(opts, { rfc6532: true, simple: true, startAt: "reply-to" }));
...
//# sourceMappingURL=email-addresses.min.js.map